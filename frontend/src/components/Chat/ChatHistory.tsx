    ...
    return (
        <ul role="list" className="space-y-0 overflow-y-auto">
            {conversations.map((item) => (
                <li key={item.id}>
                    <div className={`group flex items-center justify-between rounded-md ${item.id === chatId ? 'bg-gray-800' : 'bg-gray-900'} hover:bg-gray-800`}>
                        <Link
                            to={`/chat/${item.id}`}
                            className={`${item.id === chatId ? 'text-white' : 'text-gray-400'} group-hover:text-white flex-grow flex gap-x-3 p-2 text-sm font-semibold leading-6 min-w-0`}
                            title={item.name}
                        >
                        ...
                        <div className="flex-shrink-0 flex items-center">
                            {(item.id !== conversationToDelete || !showConfirmationDelete) ? (
                                <button
                                    ...
                                    className={`p-3 ${item.id === chatId ? 'bg-gray-800' : 'bg-gray-900'} text-gray-400 group-hover:text-white group-hover:bg-gray-800`}
                                ...
                                <div className="flex items-center p-2">
                                    <span className="text-sm text-gray-400 mr-2 whitespace-nowrap">Delete?</span>
                                    <button
                                        onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
